...
if __name__ == "__main__":
    ...
    # # ids is a list of entity IDs
    # triples_list = []
    # for i in range(len(res)):
    #     for j in range(i + 1, len(res)):
    #         id1 = res[i]
    #         id2 = res[j]
    #         result = clocq.connect(id1, id2)
    #         # Extract the third inner list of triples from the result and append it to the triples_list
    #         triples_list.extend(result[2])
    #
    # # Loop over each triple in the triples_list
    # for triple in triples_list:
    #     # Get the QID and PID from the triple
    #     qid = triple[0]
    #     pid = triple[1]
    #     # Call clocq.get_label() for the QID and PID
    #     qid_label = clocq.get_label(qid)
    #     pid_label = clocq.get_label(pid)
    #     # Print the labels
    #     print(f"QID: {qid} -> Label: {qid_label}")
    #     print(f"PID: {pid} -> Label: {pid_label}")
    # print("\n")
    #
