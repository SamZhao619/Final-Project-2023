...
def extract_sentence_sbert(facts):
    labels = [fact['label'] for fact in facts]
    sentence = ' '.join(labels)
    return sentence

def extract_sentence_bm25(facts):
    labels = [fact['label'] for fact in facts]
...
def bm25(question, entity_ids_sorted, k):
    ...
    sentences = [evidence['evidence_text'] for evidence in scored_evidences]

    # Return the first k scored evidences based on the value of k
    return sentences[:k]

def sbert(question, entity_ids_sorted, k):
    sentences = []
    for entity_id in entity_ids_sorted:
        neighborhood = clocq.get_neighborhood(entity_id)
        for fact in neighborhood:
            sentence = extract_sentence_sbert(fact)
            sentences.append(sentence)
...
def index():
    if request.method == 'POST':
        ...
        # Call the bm25 and sbert functions with the user's question and the selected value of k as inputs.
        bm25_sentences = bm25(question, entity_ids_sorted, k)
        sbert_sentences = sbert(question, entity_ids_sorted, k)

        return render_template('index.html', bm25_sentences=bm25_sentences, sbert_sentences=sbert_sentences)
